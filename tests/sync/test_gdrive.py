...
async def test_refresh_token_success():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock, return_value=mock_resp),          patch("mnemo_mcp.sync.gdrive.settings") as mock_settings:
...
async def test_refresh_token_exception():
    token = {"refresh_token": "ref"}
    with patch("httpx.AsyncClient.post", side_effect=Exception("net error")),          patch("mnemo_mcp.sync.gdrive.settings") as mock_settings:
        mock_settings.google_drive_client_id = "cid"
        mock_settings.google_drive_client_secret = "sec"
        new_token = await _refresh_token(token)
        assert new_token is None
...
async def test_get_valid_token_expired():
    ...
    with patch("mnemo_mcp.sync.gdrive._load_token", new_callable=AsyncMock, return_value=token),          patch("mnemo_mcp.sync.gdrive._refresh_token", new_callable=AsyncMock, return_value={"access_token": "new"}),          patch("mnemo_mcp.sync.gdrive._save_token", new_callable=AsyncMock):
...
async def test_get_valid_token_not_expired():
    ...
    with patch("mnemo_mcp.sync.gdrive._load_token", new_callable=AsyncMock, return_value=token):
        from mnemo_mcp.sync.gdrive import _get_valid_token
        valid = await _get_valid_token()
        assert valid["access_token"] == "live"

async def test_get_valid_token_none():
    with patch("mnemo_mcp.sync.gdrive._load_token", new_callable=AsyncMock, return_value=None):
        from mnemo_mcp.sync.gdrive import _get_valid_token
        valid = await _get_valid_token()
...
async def test_verify_folder_exists_ok():
    ...
    with patch("mnemo_mcp.sync.gdrive._drive_request", new_callable=AsyncMock, return_value=mock_resp):
...
async def test_find_or_create_folder_mem_cache():
    ...
    with patch("mnemo_mcp.sync.gdrive._verify_folder_exists", new_callable=AsyncMock, return_value=True):
...
async def test_find_or_create_folder_mem_cache_invalid():
    ...
    with patch("mnemo_mcp.sync.gdrive._verify_folder_exists", new_callable=AsyncMock, return_value=False),          patch("mnemo_mcp.sync.gdrive._load_folder_id", new_callable=AsyncMock, return_value=None),          patch("mnemo_mcp.sync.gdrive._drive_request", new_callable=AsyncMock, side_effect=Exception("stop")):
        try:
...
async def test_find_or_create_folder_disk_cache():
    with patch("mnemo_mcp.sync.gdrive._load_folder_id", new_callable=AsyncMock, return_value="disk123"),          patch("mnemo_mcp.sync.gdrive._verify_folder_exists", new_callable=AsyncMock, return_value=True):
        assert await _find_or_create_folder({}, "disk") == "disk123"

async def test_find_or_create_folder_disk_cache_invalid():
    with patch("mnemo_mcp.sync.gdrive._load_folder_id", new_callable=AsyncMock, return_value="disk_invalid"),          patch("mnemo_mcp.sync.gdrive._verify_folder_exists", new_callable=AsyncMock, return_value=False),          patch("mnemo_mcp.sync.gdrive._drive_request", new_callable=AsyncMock, side_effect=Exception("stop")):
        try:
...
async def test_find_or_create_folder_search_found():
    ...
    with patch("mnemo_mcp.sync.gdrive._drive_request", new_callable=AsyncMock, return_value=mock_resp),          patch("mnemo_mcp.sync.gdrive._load_folder_id", new_callable=AsyncMock, return_value=None),          patch("mnemo_mcp.sync.gdrive._save_folder_id", new_callable=AsyncMock):
...
async def test_find_or_create_folder_create_new():
    ...
    with patch("mnemo_mcp.sync.gdrive._drive_request", new_callable=AsyncMock, side_effect=[mock_search, mock_search, mock_search, mock_create]),          patch("mnemo_mcp.sync.gdrive._load_folder_id", new_callable=AsyncMock, return_value=None),          patch("mnemo_mcp.sync.gdrive._save_folder_id", new_callable=AsyncMock),          patch("asyncio.sleep", new_callable=AsyncMock):
...
async def test_sync_push_no_token():
    with patch("mnemo_mcp.sync.gdrive._get_valid_token", new_callable=AsyncMock, return_value=None):
        assert await sync_push(Path("db"), "folder") is False

async def test_sync_push_success(tmp_path):
    p = tmp_path / "test.db"
    p.write_bytes(b"")
    with patch("mnemo_mcp.sync.gdrive._get_valid_token", new_callable=AsyncMock, return_value={"acc": "1"}),          patch("mnemo_mcp.sync.gdrive._find_or_create_folder", new_callable=AsyncMock, return_value="fid"),          patch("mnemo_mcp.sync.gdrive._find_file_in_folder", new_callable=AsyncMock, return_value=None),          patch("mnemo_mcp.sync.gdrive._upload_file", new_callable=AsyncMock, return_value=True):
...
async def test_sync_pull_no_remote_file():
    with patch("mnemo_mcp.sync.gdrive._get_valid_token", new_callable=AsyncMock, return_value={"acc": "1"}),          patch("mnemo_mcp.sync.gdrive._find_or_create_folder", new_callable=AsyncMock, return_value="fid"),          patch("mnemo_mcp.sync.gdrive._find_file_in_folder", new_callable=AsyncMock, return_value=None):
        assert await sync_pull(Path("db"), "folder") is None
...
async def test_sync_full_no_client_id():
    from mnemo_mcp.config import settings
    with patch.object(settings, "sync_enabled", True),          patch.object(settings, "google_drive_client_id", ""):
        res = await sync_full(MagicMock())
...
async def test_sync_full_no_token():
    from mnemo_mcp.config import settings
    with patch.object(settings, "sync_enabled", True),          patch.object(settings, "google_drive_client_id", "cid"),          patch("mnemo_mcp.sync.gdrive._has_token_available", new_callable=AsyncMock, return_value=False):
        res = await sync_full(MagicMock())
        assert res["status"] == "error"
...
async def test_sync_full_token_expired():
    from mnemo_mcp.config import settings
    with patch.object(settings, "sync_enabled", True),          patch.object(settings, "google_drive_client_id", "cid"),          patch("mnemo_mcp.sync.gdrive._has_token_available", new_callable=AsyncMock, return_value=True),          patch("mnemo_mcp.sync.gdrive._get_valid_token", new_callable=AsyncMock, return_value=None):
        res = await sync_full(MagicMock())
        assert res["status"] == "error"
...
async def test_sync_full_merge_logic(tmp_path):
    ...
    with patch("mnemo_mcp.sync.gdrive.sync_pull", new_callable=AsyncMock, return_value=remote_db_path),          patch("mnemo_mcp.sync.gdrive.sync_push", new_callable=AsyncMock, return_value=True),          patch("mnemo_mcp.sync.gdrive.settings") as mock_settings,          patch("mnemo_mcp.sync.gdrive._has_token_available", new_callable=AsyncMock, return_value=True),          patch("mnemo_mcp.sync.gdrive._get_valid_token", new_callable=AsyncMock, return_value={"acc": "1"}):
        ...
        class FakeRemoteDB:
            ...
            def export_jsonl(self): return ("data\n", {})
            def close(self): pass
...
async def test_sync_full_cleanup_oserror(tmp_path):
    ...
    with patch("mnemo_mcp.sync.gdrive.sync_pull", new_callable=AsyncMock, return_value=remote_db_path),          patch("mnemo_mcp.sync.gdrive.sync_push", new_callable=AsyncMock, return_value=True),          patch("mnemo_mcp.sync.gdrive.settings") as mock_settings,          patch("mnemo_mcp.sync.gdrive._has_token_available", new_callable=AsyncMock, return_value=True),          patch("mnemo_mcp.sync.gdrive._get_valid_token", new_callable=AsyncMock, return_value={"acc": "1"}):
        ...
        with patch("mnemo_mcp.db.MemoryDB", FakeRemoteDB),              patch.object(Path, "rmdir", side_effect=OSError("busy")):
            res = await sync_full(mock_db)
            assert res["status"] == "ok"
...
def test_start_auto_sync_trigger():
    mock_db = MagicMock()
    with patch("mnemo_mcp.sync.gdrive.settings") as mock_settings,          patch("asyncio.create_task") as mock_create_task:
        mock_settings.sync_enabled = True
        mock_settings.google_drive_client_id = "cid"
        mock_settings.sync_interval = 60
...
def test_setup_sync_no_client_id():
    with patch("mnemo_mcp.sync.gdrive.settings") as mock_settings,          patch("sys.exit") as mock_exit:
        mock_settings.google_drive_client_id = ""
        setup_sync()
        mock_exit.assert_called_with(1)

def test_setup_sync_success():
    with patch("mnemo_mcp.sync.gdrive.settings") as mock_settings,          patch("mnemo_mcp.sync.gdrive.setup_google_auth", new_callable=AsyncMock, return_value=True),          patch("builtins.print"):
        mock_settings.google_drive_client_id = "cid"
        setup_sync()

def test_setup_sync_fail():
    with patch("mnemo_mcp.sync.gdrive.settings") as mock_settings,          patch("mnemo_mcp.sync.gdrive.setup_google_auth", new_callable=AsyncMock, return_value=False),          patch("sys.exit") as mock_exit,          patch("builtins.print"):
        mock_settings.google_drive_client_id = "cid"
        setup_sync()
        mock_exit.assert_called_with(1)
...
async def test_ensure_bundle_folder_search_found():
    ...
    with patch("mnemo_mcp.sync.gdrive._find_or_create_folder", new_callable=AsyncMock, return_value="base_id"),          patch("mnemo_mcp.sync.gdrive._drive_request", new_callable=AsyncMock, return_value=mock_search):
...
async def test_ensure_bundle_folder_create():
    ...
    with patch("mnemo_mcp.sync.gdrive._find_or_create_folder", new_callable=AsyncMock, return_value="base_id"),          patch("mnemo_mcp.sync.gdrive._drive_request", new_callable=AsyncMock, side_effect=[mock_search, mock_create]):
        assert await _ensure_bundle_folder({}, "folder") == "bundle_id"

async def test_ensure_bundle_folder_fail():
    with patch("mnemo_mcp.sync.gdrive._find_or_create_folder", new_callable=AsyncMock, return_value=None):
...
async def test_backend_pull_no_bundle_folder():
    backend = GDriveBackend("folder")
    with patch("mnemo_mcp.sync.gdrive._get_valid_token", new_callable=AsyncMock, return_value={"acc": "1"}),          patch("mnemo_mcp.sync.gdrive._ensure_bundle_folder", new_callable=AsyncMock, return_value=None):
        assert await backend.pull() is None

async def test_backend_pull_no_files():
    backend = GDriveBackend("folder")
    with patch("mnemo_mcp.sync.gdrive._get_valid_token", new_callable=AsyncMock, return_value={"acc": "1"}),          patch("mnemo_mcp.sync.gdrive._ensure_bundle_folder", new_callable=AsyncMock, return_value="bid"),          patch.object(GDriveBackend, "_max_sequence", new_callable=AsyncMock, return_value=0):
        assert await backend.pull() is None
...
async def test_backend_pull_max_sequence_fail():
    ...
    mock_resp = MagicMock()
    mock_resp.status_code = 500
    with patch("mnemo_mcp.sync.gdrive._get_valid_token", new_callable=AsyncMock, return_value={"acc": "1"}),          patch("mnemo_mcp.sync.gdrive._ensure_bundle_folder", new_callable=AsyncMock, return_value="bid"),          patch("mnemo_mcp.sync.gdrive._drive_request", new_callable=AsyncMock, return_value=mock_resp):
        assert await backend.pull() is None
