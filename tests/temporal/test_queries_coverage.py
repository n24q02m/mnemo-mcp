...
def test_history_for_entity_ordering(tmp_db: MemoryDB):
    ...
    tmp_db._conn.execute(
        "UPDATE memories SET created_at = '2020-01-01T00:00:00', valid_from = NULL WHERE id = ?",
        (mid1,),
    )

    mid2, _ = _seed_kg(tmp_db, "m2", [{"name": "E", "type": "concept"}])
    tmp_db._conn.execute(
        "UPDATE memories SET created_at = '2021-01-01T00:00:00', valid_from = '2021-01-01T00:00:00' WHERE id = ?",
        (mid2,),
    )

    mid3, _ = _seed_kg(tmp_db, "m3", [{"name": "E", "type": "concept"}])
    tmp_db._conn.execute(
        "UPDATE memories SET created_at = '2022-01-01T00:00:00', valid_from = '2019-01-01T00:00:00' WHERE id = ?",
        (mid3,),
    )
...
def test_history_for_entity_superseded(tmp_db: MemoryDB):
    ...
    eid = ents["E"]
    tmp_db._conn.execute(
        "UPDATE memories SET valid_to = '2023-01-01T00:00:00' WHERE id = ?", (mid,)
    )
    tmp_db._conn.commit()
...
def test_memories_as_of_limit_clamping(tmp_db: MemoryDB):
    tmp_db.add("content")
    tmp_db._conn.execute(
        "UPDATE memories SET valid_from = '2020-01-01T00:00:00' WHERE id = (SELECT id FROM memories LIMIT 1)"
    )
    tmp_db._conn.commit()
