...
# SHA256 checksums for binary verification (v1.68.2)
_RCLONE_CHECKSUMS = {
...
async def _download_rclone() -> Path | None:
    ...
    try:
        ...
        expected = _RCLONE_CHECKSUMS.get(f"{os_name}-{arch}")

        if not expected:
            logger.warning(
                f"No checksum found for {os_name}-{arch}. Skipping verification."
            )
