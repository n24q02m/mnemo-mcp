    ...
    match action:
        ...
        case _:
            ...
            resp: dict[str, typing.Any] = {
                ...
                "suggestion": "Common actions: 'add' to store new info, 'search' to find existing, 'update' to modify by ID.",
            }
            if closest:
                resp["suggestion"] = f"Did you mean '{closest[0]}'? {resp.get('suggestion', '')}".strip()
            return _json(resp)
    ...
    match action:
        ...
        case _:
            ...
            resp: dict[str, typing.Any] = {
                "error": f"Unknown action '{action}'.",
                "valid_actions": valid_actions,
                "suggestion": "Common actions: 'status' to view config, 'set' to update settings, 'sync' to manual sync.",
            }
            if closest:
                resp["suggestion"] = f"Did you mean '{closest[0]}'? {resp.get('suggestion', '')}".strip()
            return _json(resp)
        ...
        return _json(
            {
                ...
                "suggestion": "Set SYNC_PASSPHRASE env var (stdio mode) or submit the relay form passphrase field (HTTP mode) before triggering passport sync.",
            }
        )
...
async def _handle_config_export_passport(ctx: Context | None) -> str:
    ...
    if not passphrase:
        return _json(
            {
                "error": "SYNC_PASSPHRASE not set",
                "suggestion": "Set SYNC_PASSPHRASE env var or submit the relay form passphrase before exporting a passport.",
            }
        )
...
async def _handle_config_import_passport(
    ...
    if not passphrase:
        return _json(
            {
                "error": "SYNC_PASSPHRASE not set",
                "suggestion": "Set SYNC_PASSPHRASE env var or submit the relay form passphrase before importing a passport.",
            }
        )
