...
def upsert_entities(conn, entities: list[dict]) -> list[str]:
    ...
    # Use UPSERT (INSERT ... ON CONFLICT) with RETURNING for single-pass bulk write.
    # This eliminates the need for a separate SELECT pass and further reduces
    # database round-trips to O(N/BATCH_SIZE).
    BATCH_SIZE = 400
