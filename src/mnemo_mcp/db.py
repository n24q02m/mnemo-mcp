    ...
    def search(
        ...
        *,
        ...
        if embedding and self._vec_enabled:
            try:
                # Bolt Performance Optimization:
                # Consolidate N+1 queries in SQLite vector search.
                # JOIN memories_vec with memories directly to fetch all columns
                # and avoid an extra query round-trip for IDs missing in FTS results.
                vec_sql = """
                    SELECT m.*, v.distance
                    FROM memories_vec v
